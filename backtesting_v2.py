...
def main():
	...
	for s_sym in ls_symbols:
	    for i in range(1,len(ldt_timestamps)-5):
	    	...
	    	bol_clo_yes = bol_clo[s_sym].loc[ldt_timestamps[i-1]]
	    	bol_clo_tod = bol_clo[s_sym].loc[ldt_timestamps[i]]
	    	bol_vol_yes = bol_vol[s_sym].loc[ldt_timestamps[i-1]]
	    	bol_market = ts_market.loc[ldt_timestamps[i]]
	    	if bol_clo_yes >1 and bol_clo_tod < 1.5 and bol_vol_yes >1.5 and bol_market > 1.5:
	...
	for sym in ls_symbols:
		for i in range(len(ldt_timestamps)):
			if df_events[sym].loc[ldt_timestamps[i]] == 1:
	...
	'''
	df_orders = copy.deepcopy(df_events) * 100
