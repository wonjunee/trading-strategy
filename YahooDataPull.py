...
def get_yahoo_data(data_path, ls_symbols):
    ...
    for symbol in ls_symbols:
        ...
        try:
            ...
            f.close()
            ls_obtained_syms.append(symbol_name)
    ...
    return (ls_obtained_syms,ls_missed_syms)
...
def main():
    ...
    ls_symbols = dataobj.get_symbols_from_list(sym_list)
    ls_symbols.append('SPY')
    ls_symbols.append('NYA')
    path = 'C:/Python27/Lib/site-packages/QSTK/QSData/Yahoo/'
    obtained_syms, missed_syms = get_yahoo_data(path, ls_symbols)
    path = 'C:/Python27/Lib/site-packages/QSTK/QSData/Yahoo/Lists/'
    f = open (path + "mylists.txt", 'w')
    #Writing the header

    for syms in obtained_syms:
        f.write (syms + "\n")
    f.close()
